<template>
    <div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="theme in themes" :key="theme.id">
                                    <td>{{ theme.id}}</td>
                                    <td>{{ theme.name}}</td>
                                    <td>{{ theme.area.name}}</td>
                                    <td>{{ theme.description}}</td>
                                    <td>
                                        <button @click="edit(theme)" v-tooltip="'Editar'" type="button" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></button>
                                        <button @click="del(theme)" v-tooltip="'Eliminar'"  type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            </tbody>
        ...
        <modal v-show="showAddModal"  title="Formulario de creación de temas"  size="modal-md">
            <form class="form">
                ...
                <div class="form-group">
                    <label for="create_desc">Descripción</label>
                    <textarea class="form-control" id="create_desc" v-model="create.description" cols="30" rows="2"></textarea>
        ...
        <modal v-show="showEditModal"  title="Formulario de edición de temas"  size="modal-md">
            <form class="form">
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="current_desc">Descripción</label>
                    <textarea class="form-control" id="current_desc" v-model="current.description" cols="30" rows="2"></textarea>
                    <small  v-if="errors.description" class="form-text text-danger">{{ errors.description[0]}}</small>
                </div>
            </form>
        ...
        <modal v-show="showDestroyModal"  title="¿Estas seguro de eliminar esta temas?"  size="modal-md">
    ...
    export default{
        ...
        methods:{
            ...
            update(){
                ...
                Theme.update(this.current.id, this.current, data => {
            ...
            destroy() {
                Theme.destroy(this.current.id, data => {
